...
#def count_letters(text: str) -> dict:
    ...
    list_text = list(text_lower) # Приведение к списку
    list_ = list() # Создание пустового списка
    dict_ = dict() # Создание пустого словаря
    for char in list_text: # Проверка, является ли символ буквой
        if char.isalpha():
            list_.append(char)
    for char in list_: # Подсчёт количества букв
        a = list_.count(char)
        dict_[char] = a
    return dict_
...
def calculate_frequency(letter_count: dict) -> dict:
    ...
    # TODO Реализуйте функцию
#def calculate_frequency(letter_count: dict) -> dict:
    sum_alpha = sum(letter_count.values())
    for char in letter_count:
        n = letter_count[char]
        chastota = round(float(n / sum_alpha), 2)
        letter_count[char] = chastota
    return letter_count
...
for key, volue in frequency_dict.items():
    print (key, ':', ' ', f'{volue:.2f}', sep='')